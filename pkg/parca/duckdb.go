...
// setupDuckDB initializes the embedded DuckDB storage backend. It is only
// compiled into builds tagged `duckdb` because go-duckdb links a static
// libduckdb through cgo (see Dockerfile.duckdb); the untagged stub in
// duckdb_stub.go returns an error instead.
func setupDuckDB(
	ctx context.Context,
	logger log.Logger,
	flags FlagsDuckDB,
