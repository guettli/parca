...
// newDuckDBBackend initializes the embedded DuckDB storage backend. It is only
// compiled into builds tagged `duckdb` (see Dockerfile.duckdb), which enable
// cgo so go-duckdb can link its static libduckdb. The default CGO-free
// cross-platform release binary uses the stub in duckdb_stub.go instead.
