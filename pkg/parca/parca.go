...
func Run(ctx context.Context, logger log.Logger, reg *prometheus.Registry, flags *Flags, version string) error {
	...
	case "duckdb":
		// The DuckDB backend links libduckdb via cgo, so its wiring lives in
		// backend_duckdb.go (cgo) with a stub in backend_duckdb_stub.go for
		// CGO-free release builds.
		var err error
		profileIngester, querier, closeBackend, err = setupDuckDBBackend(ctx, logger, flags, tracerProvider, sharedSymbolizer)
		if err != nil {
			return err
		}
