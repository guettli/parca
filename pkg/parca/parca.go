...
func Run(ctx context.Context, logger log.Logger, reg *prometheus.Registry, flags *Flags, version string) error {
	...
	case "duckdb":
		...
		// compiled into builds tagged `duckdb` (see pkg/parca/duckdb.go). The
		// default, CGO-free cross-platform release binary returns an error here
		// via the stub in pkg/parca/duckdb_stub.go.
