...
func Run(ctx context.Context, logger log.Logger, reg *prometheus.Registry, flags *Flags, version string) error {
	...
	case "duckdb":
		// The DuckDB backend links libduckdb via cgo and is only compiled into
		// binaries built with the `duckdb` build tag (see Dockerfile.duckdb).
		// In the default CGO-free release build newDuckDBBackend is a stub that
		// returns an error instead.
		var err error
		profileIngester, querier, closeBackend, err = newDuckDBBackend(ctx, logger, tracerProvider, flags.DuckDB, sharedSymbolizer)
		if err != nil {
			return err
		}
