...
func Run(ctx context.Context, logger log.Logger, reg *prometheus.Registry, flags *Flags, version string) error {
	...
	case "duckdb":
		// The DuckDB backend depends on cgo (github.com/marcboeker/go-duckdb),
		// so it is compiled in only when built with the "duckdb" build tag. The
		// stub implementation used by the default (CGO-free, cross-compiled)
		// release build returns a clear error here. See parca_duckdb.go /
		// parca_duckdb_stub.go.
		var err error
		profileIngester, querier, closeBackend, err = initDuckDBBackend(ctx, logger, flags.DuckDB, tracerProvider, memory.DefaultAllocator, sharedSymbolizer)
		if err != nil {
			return err
		}
